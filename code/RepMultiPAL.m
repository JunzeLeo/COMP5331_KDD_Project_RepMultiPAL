%{
    ...
    Inputs:
        ...
        Mt = memory threshold.
        p = the number of previous candidate subsequences to be considered when judging the next subsequence.
    Output:
...
%}
function [multi_joins] = RepMultiPAL(data, N, m, k, L, Mt, p)
    tic;
    size_of_L = length(L);
    if size_of_L ~= 0
        for i = 1:size_of_L
            if ~isfile(strcat('matrix_profile_',int2str(L(i)),'.mat'))
                % Computing the matrix profiles for all pairs of time series
                [~, ~] = computeAllPairsMatrixProfiles(data, N, m, L(i));
            end
        end
        [multi_joins,n_join] = findKMultiJoins(N, m, k, L, Mt, p);
    else
        size_of_L = k;
        low_b = 5;
        up_b = 20;
        ...
        while iteration <= 5
            ...
            [tmp_multi_joins,tmp_n_join] = findKMultiJoins(N,m,k,L,Mt,p);
            if tmp_n_join == k 
...
function [multi_joins,n_join] = findKMultiJoins(N,m,k,L,Mt,p)
